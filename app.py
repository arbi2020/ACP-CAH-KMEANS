...
background_image = 'Tunisie-Telecom.jpg'

# Ajoutez le style CSS pour l'image de fond
st.markdown(
    f"""
    <style>
...
if submitted:
    ...
    # Create a single FacetGrid for all histograms
    grid = sns.FacetGrid(cluster_df1, col='Cluster', height=5)

    # Map histograms for each feature in the cluster_df1
    for c in cluster_df1.drop(['Cluster'], axis=1):
        grid.map(plt.hist, c, bins=20, alpha=0.7, color='steelblue')
